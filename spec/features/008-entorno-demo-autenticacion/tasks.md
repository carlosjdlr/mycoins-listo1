...
- [x] `keycloak/realm-mycoins.json`: realm `mycoins`, cliente `mycoins-api` (confidencial, PKCE S256, redirect a `/api/auth/callback`), usuarios `futbolista`, `gestor`, `administrador`.
- [x] Verificado el discovery: issuer `http://localhost:8081/realms/mycoins`, PKCE `S256` soportado.
...
- [x] Callback → **302**, cookie `mycoins_session` con `HttpOnly; Secure; SameSite=Lax`; usuario creado solo en MyCoins.
