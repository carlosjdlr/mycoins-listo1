# Guía de clase — Autenticación y Autorización en MyCoins
...
MyCoins **nunca ve la contraseña**. No existe un endpoint que reciba usuario y clave. El backend delega la identidad a un **Identity Provider** (Keycloak) mediante OAuth 2.0 Authorization Code + PKCE:

```
Navegador          MyCoins (backend)              Keycloak (IdP)
    │                     │                             │
    │  GET /api/auth/login│                             │
    ├────────────────────>│  genera state + PKCE        │
    ...
    │<────────302─────────┤                             │
    │                                                   │
    │  el usuario tipea su clave AQUÍ, no en MyCoins   │
    ├──────────────────────────────────────────────────>│
    │<─────────────302 con ?code=… &state=… ────────────┤
    │                     │                             │
...
La contraseña viaja al IdP. MyCoins sólo recibe un **código de un solo uso** y lo canjea por tokens.
...
Usuarios ya cargados en Keycloak (contraseña `mycoins123` en los tres):

| Usuario | Email | Para el rol |
|---|---|---|
| `futbolista` | futbolista@mycoins.local | Futbolista |
| `gestor` | gestor@mycoins.local | Gestor de Cancha |
| `administrador` | administrador@mycoins.local | Administrador |
...
Redirige a Keycloak → el alumno se autentica como `futbolista` / `mycoins123` → Keycloak devuelve al callback → MyCoins deja la cookie de sesión y muestra la sesión.
...
- La cookie final `mycoins_session` sale con `HttpOnly; Secure; SameSite=Lax`. `HttpOnly` significa que **JavaScript no puede leerla**: ésa es la defensa contra XSS.

El usuario **no existía** en la base de MyCoins: lo creó el callback en su primer login (`findOrSyncByOAuth`), enlazándolo al IdP por `authAccount(provider, sub)`.
...
```bash
curl -i http://localhost:3000/api/auth/session -H "Cookie: mycoins_session=<valor>"
```
...
```json
{
  "data": {
    ...
    "email": "futbolista@mycoins.local",
    "name": "Mateo Vera",
    "roles": [],
    "permissions": []
  }
...
```bash
yarn asignar-rol --email futbolista@mycoins.local --rol futbolista
```
...
```bash
yarn asignar-rol --email gestor@mycoins.local --rol gestor-de-cancha
yarn asignar-rol --email gestor@mycoins.local --rol gestor-de-cancha --sede 9d2a6897-3c6d-4ccd-8e7f-1bbdc5e01605
```
...
curl -i "http://localhost:3000/api/roles?organizationId=636a6008-f2eb-46f3-8c71-ab116e05c2a9" \
  -H "Cookie: mycoins_session=<valor>"
```
...
```bash
curl -i -X POST http://localhost:3000/api/auth/logout -H "Cookie: mycoins_session=<valor>"
```

```
HTTP/1.1 204 No Content
Set-Cookie: mycoins_session=; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Path=/
```
