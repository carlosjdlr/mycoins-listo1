...
|---|---|---|
| `estudiante` | estudiante@mycoins.local | Futbolista |
...
Redirige a Keycloak → el alumno se autentica como `estudiante` / `mycoins123` → Keycloak devuelve al callback → MyCoins deja la cookie de sesión y muestra la sesión.
...
```json
{
  "data": {
    ...
    "email": "estudiante@mycoins.local",
    "name": "Mateo Vera",
    "roles": [],
    "permissions": []
  }
