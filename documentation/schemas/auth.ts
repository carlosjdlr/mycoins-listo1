...
registry.registerComponent('securitySchemes', 'cookieAuth', {
	type: 'apiKey',
	in: 'cookie',
	name: 'mycoins_session',
});
registry.registerComponent('securitySchemes', 'bearerAuth', {
	type: 'http',
	scheme: 'bearer',
	description: 'Usado por el cliente móvil (mycoins-ionic) — ver /auth/mobile/login.',
});
...
2. Autentícate en Keycloak (p. ej. \`futbolista\` / \`mycoins123\`). **La contraseña se escribe en el IdP, nunca en MyCoins.**
...
- La cookie \`mycoins_oauth_state\` guarda \`state\`, \`nonce\` y \`code_verifier\` **cifrados**.
- La cookie final \`mycoins_session\` sale con \`HttpOnly; Secure; SameSite=Lax\`. \`HttpOnly\` = **JavaScript no puede leerla**: ésa es la defensa contra XSS.`;
...
4. Crea o sincroniza el usuario en MyCoins (\`findOrSyncByOAuth\`), enlazándolo al IdP por \`authAccount(provider, sub)\`.
5. Sella la sesión en una cookie cifrada y redirige.

> **Ojo con el orden:** el usuario de MyCoins lo crea **este** endpoint, en el primer login. Si creas el usuario antes con \`POST /users\` usando el mismo email, no tendrá \`authAccount\`, y este paso intentará crear otro usuario con ese email y fallará por unicidad.`;
...
\`\`\`json
{ "data": { "email": "futbolista@mycoins.local", "roles": [], "permissions": [] } }
\`\`\`
...
2. Fíjate en la cabecera de respuesta: \`Set-Cookie: mycoins_session=; Max-Age=0\` — así se borra una cookie.
...
\`\`\`bash
curl -i http://localhost:3000/api/auth/session -H "Cookie: mycoins_session=<valor-viejo>"
\`\`\`
...
const mobileLoginDescription = `Inicia sesión con usuario/contraseña directos desde la app móvil empaquetada (Capacitor), usando el cliente **público** \`mycoins-mobile\` con Resource Owner Password Credentials (ROPC).

**Decisión de producto explícita:** la app móvil pide usuario y contraseña en un formulario nativo propio (no abre Keycloak en un navegador aparte) — por eso \`mycoins-mobile\` es el único cliente del realm con \`directAccessGrantsEnabled: true\`. El cliente web (\`mycoins-api\`) sigue exigiendo Authorization Code + PKCE sin excepción.

**Qué hace, en orden:**

1. Envía \`username\`/\`password\` a Keycloak como grant \`password\` (ROPC) con el cliente público \`mycoins-mobile\`.
...
3. Verifica el \`id_token\` devuelto (firma, issuer, audience = \`mycoins-mobile\`) — sin \`nonce\`, porque ROPC no tiene handshake de redirección al que fijárselo.
...
> **Riesgo aceptado explícitamente:** ROPC expone la contraseña al código de la app (no solo a Keycloak) y pierde compatibilidad sencilla con MFA/social login futuro. Se activó únicamente para \`mycoins-mobile\`, como decisión de producto documentada, no como negligencia.`;
