...
vi.hoisted(() => {
	...
	process.env.DATABASE_URL = 'postgresql://user:password@localhost:5432/mycoins?schema=public';
	...
	process.env.OAUTH_MOBILE_CLIENT_ID = 'mycoins-mobile';
	...
	process.env.SESSION_COOKIE_NAME = 'mycoins_session';
	process.env.SESSION_TEMP_COOKIE_NAME = 'mycoins_oauth_state';
...
describe('auth mobile login route (ROPC, cliente móvil)', () => {
	...
	it('logs in with username/password using the public mobile client and returns a sessionToken', async () => {
		...
		await handler(buildRequest({ username: 'futbolista', password: 'mycoins123' }), response);

		expect(passwordGrant).toHaveBeenCalledWith('futbolista', 'mycoins123');
		expect(createSession).toHaveBeenCalledWith(
			'11111111-1111-1111-1111-111111111111',
			expect.objectContaining({ accessToken: 'access-token', clientId: 'mycoins-mobile' }),
		);
